 ...
 * Created by PhpStorm.
 * Author: Shengjun
 * CreateTime: 15-3-19.下午4:38
 * Description:
 * 用于管理application  crontab 任务
 * 使用方法：
 *  1. console config/bootstrap.php 添加
 * Yii::setAlias('@runnerScript', dirname(dirname(dirname(__FILE__))) .'/yii');
 * 2. 添加计划任务
 *  1. windows 添加计划任务  每分钟执行 {path-to-application}/yii cron
 *  2. linux  crontab 添加  * * * * * {path-to-application}/yii cron
 * 3. 添加job
 *
 *
 */
...
class CronController extends Controller {
    ...
    public function getHelp() {
        $commandUsage = Yii::getAlias('@runnerScript').' '.$this->getUniqueId();
        return <<<RAW
        ...
        foreach ($parameters AS $n => &$repeat) {
            ...
            if ($repeat === '*') $repeat = range($dimensions[$n][0], $dimensions[$n][1]);
                ...
                foreach (explode(',', $repeat) as $piece) {
                    ...
                    $piece = explode('-', $piece, 2);
                    if(count($piece) == 2){
                        $repeatPiece = array_merge($repeatPiece, range($piece[0], $piece[1],$step));
                    }elseif($piece[0] === '*'){
                        $repeatPiece = array_merge($repeatPiece, range($dimensions[$n][0], $dimensions[$n][1],$step));
                    }else{
                        $repeatPiece[] = $piece[0];
                    }
                }
                $repeat = array_unique($repeatPiece);
            }
    ...
    protected function prepareActions()
    {
        ...
        if (!empty($methods)) {
            foreach ($methods as $runCommand => $runSettings) {
                ...
                }elseif (count($runCommand) == 3) {
                    $actions[] = array(
                        'command' => $runCommand[0] . '/' . $runCommand[1],
                        'action'  => $runCommand[2],
                        'docs'    => $this->parseDocComment($this->arrayToDocComment($runSettings))
                    );
                }elseif (count($runCommand) == 1) {
                    $actions[] = array(
                        'command' => $runCommand[0],
                        'action'  => 'index',
                        'docs'    => $this->parseDocComment($this->arrayToDocComment($runSettings))
