 ...
 *
 * Extend onSelect parameters.
 * Modify by Jimmy, Date: 2015/7/29
 */
var STComboBox = (function($) {
    ...
    Class.prototype.onKeyDown = function(evt) {
        if(evt.which == keycodes.enter) {
            ...
            this.hideList();
            if(this.onSelect) {
                this.onSelect(evt, this.getInput().val(), this.data[selectedIndex]);
            }
            return false;
        }
    ...
    Class.prototype.populateList = function(data) {
        ...
        $list.find('.stc-lrow').hover(function() {
            ...
            if(self.onSelect) {
                self.onSelect(evt, self.getInput().val(), self.data[selectedIndex]);
            }
        });
        this.visibleRows = this.getDomRows();
        this.selectedIndex = -1;
